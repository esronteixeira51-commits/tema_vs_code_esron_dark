...
- **Python (confirmado, testado com Pylance ativo)** — com Pylance rodando, o comportamento é sólido: variável comum fica vermelho vivo, parâmetro fica branco, e *type hints* (`nome: str`, `idade: int`) ficam roxo — o único lugar onde tipo primitivo aparece em Python. Uma variável de módulo com anotação de tipo e nunca reatribuída (ex.: `LIMITE_MAXIMO: int = 999`) é inferida como readonly automaticamente e ganha negrito, mesmo sem `const` existir na linguagem. Duas particularidades que não são bugs: (1) decorators como `@dataclass` ou um decorator seu (`@obsoleto`) saem na cor de **função** (azul), não na de decorator (laranja) — porque em Python um decorator é literalmente uma chamada de função, e o Pylance não emite uma categoria semântica separada pra isso; (2) `@property` e `@staticmethod` saem no **salmão** (cor de tipo), porque são classes embutidas do Python por baixo. TypeScript confirmado com o mesmo comportamento do Python (semantic token `function`, sem itálico) — ainda não temos nenhum caso confirmado por inspector de uma linguagem onde decorator realmente vire uma categoria semântica separada.
- **C#** — confirmado por inspector: atributos (`[Obsolete]`, `[Serializable]`...) saem na cor de tipo (salmão, `#D16D6D`), não branco como registrado antes. Scope real é `entity.name.type.cs`, genérico demais pra diferenciar de uma referência de tipo comum — limitação confirmada da gramática do C#, não um gap corrigível. Ver `docs/roadmap.md`.
- **Rust** — `#[derive(...)]` era registrado como "confirmado funcionando" com base em observação visual; checagem com o inspector mostrou que era um engano — o `rust-analyzer` não emite semantic token pra isso, e o scope TextMate real (`meta.attribute.rust`) não batia com nenhuma regra do tema. Corrigido adicionando `meta.attribute` na regra de decorators **— confirmado visualmente**: `derive` agora pega o laranja itálico corretamente. Tipos primitivos usam um token não-padrão do `rust-analyzer` (`builtinType`); sem ele ativo, cai no fallback TextMate.
